...
% This script plots switches of the brightes measured polar fluorescence
% intensity, intended use is to compare an early and a late timepoint. 

%% Pre-run Settings

% Search all fields with "directory" and replace with your personal directories
% Rename only strings but no variables!
% !!! Also rename all the directories of functions\save_displacement_maps.m
% !!!

dir_func='C:\directory\'; % directory of the Functions folder, note capital F, same folder as for basic analysis
save_dir = 'G:\directory\pole2pole_oscillations\'; % directory for saving the plots, must contain "mat_files", "svg_files", "fig_files" folder

%% Enter dreams, desires and wishes

combined = 1; % 1 for moving and non-moving cells
move = 0;  % 1 for moving cells only, set combined to 0
% both 0 for non-moving
filtered = 1; % filters out pole2pole switches that occur in subsequent frames, i.e. that last just one frame

time_early = "10min"; % has to match the "early" interval
time_late = "70min"; % has to match the "late" interval
...
only_plot = 0; % if 1, loads the mat file that is saved when running with setting 0, enter name of mat file below: data_name
do_save = 1; % if 0, doesn't save the graphs

% decide which plots to plot and save
plot_histograms = 1; % plots histograms of pole-to-pole switches
plot_fracosc = 1; % plots fraction of cell tracks with zero pole-to-pole switches
plot_freqtot = 1; % plots the frequency of pole-to-pole switches, all detected switches divided by the total tracked time of all cell tracks
plot_freqcellmean = 0; % plots the average frequency of pole-to-pole switches per cell track
...
if ~only_plot
    ...
    data_name = 'mat_file_name'; % if only_plot = 1 copy the name of the mat file you want to plot WITHOUT .mat
