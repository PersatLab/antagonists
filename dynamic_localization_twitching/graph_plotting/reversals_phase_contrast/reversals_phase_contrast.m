...
save_dir = 'G:\directory\reversals_phase_contrast\'; % directory for saving the plots, must contain "mat_files", "svg_files", "fig_files" folder

%% To modify:
title_graph = "Time on surface = 2h"; %    Min limit time=2   Reversal time=5';

% only ativate one of the following options!!!
reversals_only = 0; % plots only filtered reversals, filtering never worked well, not used anymore
jiggles_only = 0; % plots only filtered jiggles, filtering never worked well, not used anymore
reversals_jiggles = 1; % plots reversals and jiggles all combined, that's what we consider a reversal, although it's not optimal
...
y_revs = 55; % for reversal frequencies typically 55, for number of reversing cells typically below 1
...
more_than_SIX_replicates = 0; % if more than 6 replicates the colours are all black, if not set to 1 produces error when more than 6 replicates
