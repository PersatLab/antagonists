...
if ~only_plot
    tic
    clearvars -except strainFolder do_save only_plot do_violin fluo dir_save_mat dir_save_graph dir_data y_limit_profiles y_max_fluo y_max_polLoc y_max_asym; close all; clc;
    ...
    % load functions
    dir_func = 'C:\directory\Functions';
    addpath(dir_func);

    % Read Input file (Format of columns must be: Strain number - Strain name - date - condition) | Note: condition names must all have the same number of characters | Filenames must be prepared by SplitImage ImageJ macro with correct sorting into folders
    ...
    % Save ALL
    clearvars -except fluo do_violin do_save only_plot dir_save_mat dir_save_graph bio_reps data_root_path dates conditions conditions_unique nbr_conditions dates_unique MegaStruct nbr_PTU NbUpSamples Pil_numbers Pil_numbers_unique Pil_types Pil_types_unique y_limit_profiles y_max_fluo y_max_polLoc y_max_asym 
%     if do_save
        save_name = 'Data';
...
if only_plot
    ...
    y_limit_profiles_onlyplot = y_limit_profiles;
    y_max_fluo_onlyplot = y_max_fluo;
    y_max_polLoc_onlyplot =  y_max_polLoc;
    y_max_asym_onlyplot = y_max_asym;
    clearvars -except fluo load_name strainFolder do_violin do_save_onlyplot dir_save_mat dir_save_graph y_limit_profiles_onlyplot y_max_fluo_onlyplot y_max_polLoc_onlyplot y_max_asym_onlyplot
    load(strcat(dir_save_mat,'\',load_name,'.mat'));
    do_save = do_save_onlyplot;
    y_limit_profiles = y_limit_profiles_onlyplot;
    y_max_fluo = y_max_fluo_onlyplot;
    y_max_polLoc =  y_max_polLoc_onlyplot;
    y_max_asym = y_max_asym_onlyplot;
    clear load_name
    % load functions
    dir_func = 'C:\directory\Functions';
    addpath(dir_func);
...
colours = [236 32 36; 0 166 156; 139 197 63; 242 145 0; 29 112 183; 229 0 126; 100 100 100; 0 0 0];
if nbr_PTU > length(colours) | nbr_conditions > length(colours)
...
for cond = 1:nbr_conditions
    ...
    axis([0 position+1 0 y_max_fluo])
...
for cond = 1:nbr_conditions
    ...
    axis([0 position+1 y_min y_max_polLoc])
    set(gca, 'XTickLabel',["";xlabels;""], 'Fontsize',10, 'TickLabelInterpreter','none')
    xticks([0:position+1])
    xtickangle(15)
...
for cond = 1:nbr_conditions
    ...
    axis([0 position+1 y_min y_max_asym])
    set(gca, 'XTickLabel',["";xlabels;""], 'Fontsize',10, 'TickLabelInterpreter','none')
    xticks([0:position+1])
    xtickangle(15)
